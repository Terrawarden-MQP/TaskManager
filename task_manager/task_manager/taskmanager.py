...
class TaskManagerNode(Node):
    ...
    def __init__(self):
        ...
        self.drone_params = {
            ...
            "precision_max_lin_vel_m_s": 0.0,
            "precision_max_ang_vel_deg_s": 0.0,
            "precision_max_lin_accel_m_s2": 0.0,
            "precision_max_z_vel_m_s": 0.0,
        }
        ...
        self.border_points = []
        self.approach_point_offset = Point(x=-0.3, y=0.0, z=0.34)
...
# ----- DRONE HELPERS
    ...
    def FLU2NED_quaternion(self, FLUoffsetPoint: Point, timestamp: Time = None) -> Point:
        ...
        # self.debug(self.debug_drone, f"Timestamp of extract cluster: {timestamp}")
        # self.debug(self.debug_drone, f"Quaternion of drone (from task manager): {quat}")
            
        quat_np = np.array([quat.w, quat.x,  quat.y, quat.z])
        quat_np_conjugate = np.array([quat.w, -quat.x, -quat.y, -quat.z])
        
        # quaternion multiplication is commutative - perform the active rotation
        rotated_point = multiply_quaternion(multiply_quaternion(quat_np_conjugate, point_np), quat_np)
        
        # extract the rotated point
        NED_offset_point = Point(x=rotated_point[1], y=rotated_point[2], z=rotated_point[3])
        
        # self.debug(self.debug_drone, f"NED Offset Point: {NED_offset_point}")
...
# ----- SEARCH
    def search(self) -> State:
        ...
        if self.is_new_data_from_subscriber(self.extract_subscriber):
            ...
            self.debug(self.debug_drone, f'found object at ({self.extract_pt.point}), turning towards object')                     
            ...
            # self.debug(self.debug_drone, f'Sending drone setpoint at ({FLU_pos})')                                    
            NED_pos = self.FLU2NED_quaternion(FLU_pos)#, self.extract_pt.header.stamp) 
            # self.debug(self.debug_drone, f'Which is ({NED_pos}) in NED (FLU2NED_quaternion)')                     
            ...
            return self.set_wait(State.NAVIGATING, 10) # Move to nav after 5 seconds 
...
# ----- NAVIGATE
    def navigate(self) -> State:
        ...
        if self.is_new_data_from_subscriber(self.extract_subscriber):
            ...
            # convert that 3D point to NED, offset it above and towards the drone a bit        
            FLU_pos = self.offsetPointFLU(self.extract_pt.point, self.approach_point_offset)
            # divide FLU_pos by approach_scaling to slow approach
            approach_scaling = 3.
            FLU_pos.x = FLU_pos.x / approach_scaling
            FLU_pos.y = FLU_pos.y / approach_scaling
            FLU_pos.z = FLU_pos.z / approach_scaling
            NED_pos = self.FLU2NED_quaternion(FLU_pos)#, self.extract_pt.header.stamp) 
            ...
            if self.isInRangeNED(NED_pos, 0.08, 0.08):  
                self.debug(self.debug_vbm, f'within range of object, changing to GRASPING')               
                return self.set_wait(State.TRACKING, 1) # Move to grasp after 5 seconds
...
# ----- GRASP
    def grasp(self) -> State:
        ...
        # in theory meters - these should be class variables but i'm still testing them
        # See more info in #manipulators  -K
        if posDiff < 0.04: # TODO uncomment
        ...
        if self.arm_status.grasping_object == True:
            ...
            return self.set_wait(State.HOLD, 3)
...
# ----- DEPOSIT
    def deposit(self) -> State:
        ...
        # measured home position
        home_pose.pose.position.x = 0.2945
        home_pose.pose.position.y = 0.01
        home_pose.pose.position.z = -0.4921
        self.sendArmCommand(home_pose, task_space="task", grasp_at_end_of_movement=False) # TODO uncomment
